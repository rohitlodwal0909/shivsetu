...
import { FaMapMarkerAlt, FaStar, FaArrowRight, FaOm } from 'react-icons/fa';
import SafeImage from '../../../components/common/SafeImage';

const ChadavaCard = ({ id, image, name, temple, location, price, rating, reviews }) => {
    const navigate = useNavigate();

    return (
        <div
            onClick={() => navigate(`/booking/chadava/${id}`)}
            className="group relative bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer h-[420px] border border-gray-100"
        >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 z-20 translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                ...
                {/* Title */}
                <h3 className="text-2xl font-bold text-white mb-1 leading-tight drop-shadow-md group-hover:text-orange-50 transition-colors">
                    {name}
                </h3>
                ...
                <div className="flex items-center justify-between border-t border-white/20 pt-4 mt-4">
                    <div className="flex flex-col">
                        <span className="text-gray-300 text-xs uppercase tracking-wide">Offering</span>
                        <div className="flex items-center gap-1.5 text-white font-medium">
                            <FaOm className="text-orange-400" /> Sacred
                        </div>
                    </div>

                    <div className="flex flex-col items-end">
                        <span className="text-gray-300 text-xs uppercase tracking-wide">Contribution</span>
